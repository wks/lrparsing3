...
class TestLRParsing(unittest.TestCase):
    maxDiff=None
    ...
    def test_repr(self):
        ...
        for rule, repr_start  in test_repr_cases:
            ...
            self.assertEqual(rhs, repr_start, (i, rhs))
            i += 1
        #
        # Hard to get to special cases.
        #
        self.assertEqual(repr(Ref("x")), "Ref('x')", repr(Ref("x")))
        class G(Grammar):
            class T(TokenRegistry):
                t = Token("x")
            START = (T.t << T.t) + T.t
        self.assertEqual(repr(G.T.t), "T.t='x'", repr(G.T.t))
        self.assertEqual(repr(G.START), "START = (T.t='x' << T.t='x') + T.t='x'", repr(G.START))
        #
        ...
        self.assertEqual(G.repr_grammar(), G.result_repr_grammar, repr(G.repr_grammar()))
        G.compile_grammar()
        self.assertEqual(G.repr_productions(), G.result_repr_productions, repr(G.repr_productions()))
        self.assertEqual(G.repr_parse_table(), G.result_repr_parse_table, repr(G.repr_parse_table()))
        G.pre_compile_grammar()
        self.assertEqual(G.repr_parse_table(), G.result_repr_parse_table_pre, repr(G.repr_parse_table()))
        self.assertEqual(G.repr_parse_table(0), G.result_repr_parse_table_pre_0, repr(G.repr_parse_table(0)))
        self.assertEqual(G.repr_parse_table(-1), G.result_repr_parse_table_pre_1, repr(G.repr_parse_table(-1)))
        #
        ...
        self.assertEqual(G.repr_parse_table(), '', G.repr_parse_table())
        parse_tree = G.parse("xyzwz")
        result = G.repr_parse_tree(parse_tree)
        self.assertEqual(result, G.result_repr_parse_tree_1, repr(result))
        result = G.repr_parse_tree(parse_tree, False)
        self.assertEqual(result, G.result_repr_parse_tree_2, repr(result))
        result = G.repr_parse_tree(parse_tree[1][1])
        self.assertEqual(result, "'x'", result)
        #
        ...
        try:
            G.compile_grammar()
            self.fail("GrammarError expected because Grammar is ambiguous")
        except lrparsing.GrammarError as e:
            pass
        result = G.repr_parse_table()
        self.assertEqual(result, G.repr_parse_table_result, repr(result))
        #
        # Check unused_rules.
        #
        class G(Grammar):
            ...
            START = Token("x")
        G.compile_grammar()
        self.assertEqual(G.unused_rules(), frozenset((G.unused,)), G.unused_rules() )
        #
        ...
        class G(Grammar):
            START = Token("x")
        self.assertEqual(str(G.epoch_symbol()), "<G>", repr(str(G.epoch_symbol())))
    ...
    def test_dict(self):
        class G(Grammar):
            ...
            y["y"] = "yy"
            START = y
        self.assertEqual(G.T.t["t"], "z")
        self.assertEqual(G.T.t.dict, {"t": "z"}, G.T.t.dict)
        self.assertIn('c', G.z, G.z.dict)
        del G.z['c']
        self.assertEqual(G.z.dict, {"b": "q"}, G.z.dict)
        self.assertEqual(G.y.dict, {"y": "yy"}, (G.y.dict, G.START.dict))
        self.assertEqual(G.START.dict, {})
        #
        ...
        class G(Grammar):
            ...
            A0["a"] = "b"
            START = A0
        self.assertEqual(G.A0.dict, {"a": "b"}, G.A0.dict)
        self.assertEqual(G.START.dict, {}, G.START.dict)
        self.assertTrue(G.START, bool(G.START))
    ...
    def test_compile_symbol(self):
        ...
        for rule, repr_productions  in test_compile_symbol_cases:
            ...
            try:
                ...
                self.assertEqual(result, repr_productions, (i, result))
            except:
    ...
    def test_build_lr1_table(self):
        ...
        self.assertEqual(reduce_1, reduce_2)
        self.assertNotEqual(reduce_1, reduce_3)
    ...
    def test_pre_compile(self):
        ...
        self.assertIsNone(G.pre_compile_grammar(result))
        self.assertIsNone(G.pre_compile_grammar(result))
        G.compile_grammar()                 # Flush pre-compile
        self.assertIsNone(G.pre_compile_grammar(eval(result)))
    ...
    def test_tokenisation(self):
        ...
        result = G.repr_parse_tree(parse_tree)
        self.assertEqual(result, "(START 'x' 'xx' 'x' T.user 'y y' 'x' T.user)", result)
        #
        ...
        result = G.repr_parse_tree(parse_tree)
        self.assertEqual(result, "(START '1' ',' T.token9 ',' '0' 'A' ',' 'B' ',' 'b')", result)
        #
        ...
        result = G.repr_parse_tree(parse_tree)
        self.assertEqual(result, "(START 'x')")
        #
        ...
        self.assertEqual(result, G.repr_parse_tree_result, repr(result))
    ...
    def test_lr1_parser(self):
        ...
        try:
            ...
            self.assertIn('line 1 column 2: Got T.x when expecting T.y while trying to match START in state 2', str(e), repr(str(e)))
    ...
    def test_grammar_errors(self):
        ...
        for error_re, grammar_dict  in test_grammar_error_tests:
            with self.assertRaises(lrparsing.GrammarError) as cm:
                dct = grammar_dict() if isinstance(grammar_dict, collections.Callable) else grammar_dict
                grammar = type('G', (Grammar,), dct)
                grammar.compile_grammar()
                self.fail((i, error_re))
            match = re.search('(?is)' + error_re, str(cm.exception).lower())
            self.assertNotEqual(match, None, (i, error_re, str(cm.exception)))
            i += 1
